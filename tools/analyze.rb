...
results[(all ? 0 : -1)..-1].each do |dir|
  ...
  if $0 == __FILE__
    ...
    puts "Quota-based delivery Criterion"
    puts "  #{$traffic.numberOfExpectedBundles(:quota => 10)} expected bundles"
    puts "  #{$traffic.deliveryRatio(:quota => 10) * 100}% deliveryRatio"
    puts "Reachability Checking Delivery Criterion"
    puts "  #{$traffic.numberOfExpectedBundles(:net => $network)} expected bundles"
    puts "  #{$traffic.deliveryRatio(:net => $network) * 100}% deliveryRatio"
