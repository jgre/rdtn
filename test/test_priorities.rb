...
class TestPriorities < Test::Unit::TestCase
  ...
  def setup
    ...
    @bundles.each_with_index {|b, i| b.creationTimestamp = (Time.now - Time.gm(2000)).to_i + i*10}
    ...
    @subscriptions = Uris.map {|uri| Subscription.new(uri)}
    usub0local = UniqueSubscription.new(nil)
    usub0remote = UniqueSubscription.new(nil,false,Time.now,Time.now+3600,10)
    usub1local = UniqueSubscription.new(nil,true,Time.now-3600,Time.now+3600)
    usub1remote = UniqueSubscription.new(nil,false,Time.now-3600,Time.now+3600,10)
    usub2local = UniqueSubscription.new(nil)
    usub2remote = UniqueSubscription.new(nil,false,Time.now,Time.now+3600,10)
  ...
  def test_not_subscribed_filter
    filter = KnownSubscriptionFilter.new
    res1=@bundles.find_all {|bundle| filter.filterBundle?(bundle,Neighbor1Eid)}
