...
RDTN bundle objects are implemented by {\tt Bundle}. The objects
contain a list of Bundle blocks, the first of which is always the primary bundle
block (implemented by {\tt PrimaryBundleBlock}). The list of blocks
also contains an arbitrary number of objects derived from {\tt Block},
which includes the payload block ({\tt PayloadBlock}). The blocks
...
processing flags are implemented as subclasses of Block.
PayloadBlock is a subclass of Block implementing the container format
...
It is implemented by {\tt ForwardLog}, storing an entry for each time
...
BundleWorkflow. The workflow object is notified of new data from the
...
Settings which is passed to all components in the node that require
...
as it is for ``real'' nodes connected over a network.
...
the nodes. The core together with the nodes and the simulation convergence
layer provides the infrastructure for simulating delay-tolerant networks; in
order to run a simulation, a {\em network model} and a {\em workload} are
needed. In the following sections we present how the core manages the events and
the passage of simulation time, how the simulation convergence layer works, and
how network models and workloads are used for running simulations.

\subsection{Event Processing and Timing}\label{sec.sim.events}

As the RDTN simulation module is a discrete event simulator is deals with a
...
When the simulation core receives a {\tt :simConnection} event, it creates a new
memory link between the nodes attached to the event. This link can be used by
both nodes like any other bi-directional convergence layer link to send and
receive bundles. The link is taken down when the core receives a {\tt
:simDisconnection} event for the connected nodes.
...
limits, only the number of bytes that can be sent in a second are copied to the
receiving link object, the rest of the bundle remains in the queue. In order to
transmit all the queued data, the link add a new event for the simulator event
queue that is scheduled for the next second. When that event is received, the
next chunk the can be transmitted in one second is copied.
...
a link to the node identified by the second one.
...
daemon, the programmed action can be implemented using the functions show for
